...
app.controller('congressDBMainController', function congressDBMainController($scope, $rootScope, $http, $filter) {
    ...
    $http.get('congress.php').success(function(data) {
        ...
        $scope.resultDataLegByHouse = $filter('filter')($scope.resultDataLegByState, {
            chamber: 'house'
        }, true);
        $scope.resultDataLegBySenate = $filter('filter')($scope.resultDataLegByState, {
            chamber: 'senate'
        }, true);
    });
    ...
    $scope.committee = function() {
        ...
        $scope.getCommTabData = function() {
            ...
            req.success(function(data) {
                ...
                $scope.commDataHouse = $filter('filter')($scope.commDataResult, {
                    chamber: 'house'
                }, true);
                $scope.commDataSenate = $filter('filter')($scope.commDataResult, {
                    chamber: 'senate'
                }, true);
                $scope.commDataJoint = $filter('filter')($scope.commDataResult, {
                    chamber: 'joint'
                }, true);
    ...
    $scope.IsFavComm = function(commRowData) {
        $scope.temp = [];
        $scope.temp = $filter('filter')($rootScope.fav_comm, {
            committee_id: commRowData.committee_id
        }, true);
    ...
    $scope.IsFavLeg = function(legRowData) {
        ...
        $scope.temp = [];
        $scope.temp = $filter('filter')($rootScope.fav_leg, {
            bioguide_id: legRowData.bioguide_id
        }, true);
        return ($scope.temp.length > 0);
    }
    ...
    $scope.IsFavBill = function(billRowData) {
        $scope.temp = [];
        $scope.temp = $filter('filter')($rootScope.fav_bill, {
            bill_id: billRowData.bill_id
        }, true);
        return ($scope.temp.length > 0);
    }
